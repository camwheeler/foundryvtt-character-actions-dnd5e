...
import { error, log } from "./lib/lib";
import { MySettings } from "./dnd5eCharacterActions-model";

Handlebars.registerHelper(`${CONSTANTS.MODULE_ID}-isEmpty`, (input: Object | Array<any> | Set<any>) => {
...
Handlebars.registerHelper(`${CONSTANTS.MODULE_ID}-isItemInActionList`, API.isItemInActionList);
...
async function addActionsTab(
	...
	if (data instanceof Promise) {
		error("data was unexpectedly a Promise, you might be using an unsupported sheet");
		return;
	}
...
Hooks.once("init", async function () {
	log(`Initializing ${CONSTANTS.MODULE_ID}`);

	// Register custom module settings
	registerSettings();

	// Preload Handlebars templates
	await loadTemplates(Object.values(flattenObject(CONSTANTS.TEMPLATES)));

	const characterActionsModuleData = game.modules.get(CONSTANTS.MODULE_ID);
	...
	// TODO TO REMOVE
	globalThis[CONSTANTS.MODULE_ID] = {
		renderActionsList: async function (...args) {
			log(`api: ${characterActionsModuleData?.api}`);
...
Hooks.on("renderActorSheet5e", async (app, html, data) => {
	...
	switch (app.actor.type) {
		case "npc": {
			const injectNPCSheet = game.settings.get(CONSTANTS.MODULE_ID, MySettings.injectNPCs) as boolean;
			if (!injectNPCSheet) {
				return;
			}
			break;
		...
		case "vehicle": {
			const injectVehicleSheet = game.settings.get(CONSTANTS.MODULE_ID, MySettings.injectVehicles) as boolean;
		...
		case "character": {
			const injectCharacterSheet = game.settings.get(CONSTANTS.MODULE_ID, MySettings.injectCharacters) as boolean;
	...
	log(`default sheet open hook firing ${app} ${html} ${data}`);

	const actionsList = $(html).find(".character-actions-list-5e");

	log(`actionsListExists ${actionsList.length}`);
